...
flight =  std_atmosphere(2850)        # One input: z - altitude
...
print(flight.Reynolds(0.305,10))        # two inputs: reference length and air speed
...
densities =  [std_atmosphere(z).density() for z in Z]
...
Mach_numbers = [std_atmosphere(1500).Mach(u) for u in range(10,50,5)]

# Reynolds number at differente flight speeds and altitudes

Z = [0,1500,3000,4500,10000]   # For each altitude, there is a corresponding flight speed
U = [30,35,40,45,50]

Reynolds_numbers = [std_atmosphere(z).Reynolds(0.2,U[x]) for x,z in enumerate(Z)]
